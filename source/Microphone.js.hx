...
class Microphone extends EventDispatcher
{
    ...
    function onAudioProcess(event:Dynamic):Void {
        ...
        if (PlayState.roome != null && inputData != null) {
            ...
            if (times >= 50) {
                times = 0;
                PlayState.roome.send("proximitychat", cur + "");
                cur = "";
            }
        }
    }
    public function playAudio(audioData:Float32Array, volume:Float, source:AudioBufferSourceNode, x=0.0,y=0.0):Void {
        if (source != null) {
            source.disconnect();
        }
        ...
        pannerNode.setPosition(dx/100, dy/100, 0);
